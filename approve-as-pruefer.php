...
if(isset($_POST['no']) AND $_POST['no'] == "No"){
	try {
	    ...
	    $parameterString = "";
		$symptomUpdateQuery="UPDATE temp_quelle_import_test SET pruefer_priority = 0 WHERE id = '".$_POST['symptom_id']."'";
		$db->query($symptomUpdateQuery);

		// Deleteing the temp pruefers because they are no longer need if it is Not approved as Pruefer.
	...
	if(isset($_POST['suggested_pruefer']) AND $_POST['suggested_pruefer'] != ""){
		...
		try {
		    ...
		    // Cleaning previous temp pruefers because below we will insert fresh
			$deleteTempPrueferQuery = "DELETE FROM temp_pruefer WHERE symptom_id = '".$_POST['symptom_id']."'";
			$db->query($deleteTempPrueferQuery);
			...
			$prueferQuery = "INSERT INTO temp_symptom_pruefer (symptom_id, pruefer_id) VALUES ('".$_POST['symptom_id']."', '".$_POST['suggested_pruefer']."')";
			$db->query($prueferQuery);

			// Storing in temp_approved_pruefer for searching in case of a. a. O.
			$tempApprovedPrueferQuery = "INSERT INTO temp_approved_pruefer (master_id, symptom_id, pruefer_id, approval_string) VALUES ('".$_POST['master_id']."', '".$_POST['symptom_id']."', '".$_POST['suggested_pruefer']."', NULLIF('".$_POST['approval_string']."', ''))";
			...
			// Deleteing the temp remeds because they are no longer need if it is approved as Pruefer.
	...
	try {
	    ...
	    if($isPruferFoundInDb == 1 AND (isset($prueferArray[0]['pruefer_id']) AND $prueferArray[0]['pruefer_id'] != "")){
			...
			// Inserting freshly, selected Pruefer
			$prueferQuery = "INSERT INTO temp_symptom_pruefer (symptom_id, pruefer_id) VALUES ('".$_POST['symptom_id']."', '".$foundPrueferId."')";
			$db->query($prueferQuery);

			// Storing in temp_approved_pruefer for searching in case of a. a. O.
			$tempApprovedPrueferQuery = "INSERT INTO temp_approved_pruefer (master_id, symptom_id, pruefer_id, approval_string) VALUES ('".$_POST['master_id']."', '".$_POST['symptom_id']."', '".$foundPrueferId."', NULLIF('".$_POST['approval_string']."', ''))";
			$db->query($tempApprovedPrueferQuery);
			...
			if($mainPrueferId != ""){
				...
				// Inserting freshly created PrueferId with this Symptom
				$symptomPrueferQuery = "INSERT INTO temp_symptom_pruefer (symptom_id, pruefer_id) VALUES ('".$_POST['symptom_id']."', '".$mainPrueferId."')";
				$db->query($symptomPrueferQuery);

				// Storing in temp_approved_pruefer for searching in case of a. a. O.
				$tempApprovedPrueferQuery = "INSERT INTO temp_approved_pruefer (master_id, symptom_id, pruefer_id, approval_string) VALUES ('".$_POST['master_id']."', '".$_POST['symptom_id']."', '".$mainPrueferId."', NULLIF('".$_POST['approval_string']."', ''))";
				$db->query($tempApprovedPrueferQuery);
